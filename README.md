...
**Windex and Scorekeeper are separate apps.** Windex is its own product: a **points ledger + standings aggregation platform** for golf, with its own API and (in progress) UI. Windex does not compute golf competition formats (e.g. Stableford, match play, best ball, skins); external systems or human admins determine points. Scorekeeper is a separate golf scoring app and may use the Windex API to submit rounds; that is Scorekeeper’s only relation to Windex. **This directory** (`Desktop\windex`) holds the Windex documentation, admin UI, API, and mobile app: **`windex-api/`** (Supabase backend), **`windex-admin/`** (Vite + React admin web UI), **`windex-expo/`** (Expo app — web via `expo start --web`, iOS/iPad via Expo Go), root-level product docs, **`patches/`** (WIP patches), and **`glide-export/`** (Glide ODS export).
...
1. **Web admin UI first** — Run **`windex-admin/`** against your API (local or hosted). Confirm flows, data, and auth there before moving on.
2. **Then mobile (iPad / iPhone)** — After the web UI checks out, exercise the same backend through **`windex-expo/`** (Expo Go). The mobile app is a thinner client; fixing issues in admin first avoids chasing device-only problems when the API or data is wrong.

**Mobile (Expo Go):** From this repo folder run `npm install` once inside `windex-expo`, then from **there** run `npx expo start --tunnel` to open the app on iPhone/iPad with Expo Go. Details: [windex-expo/README.md](./windex-expo/README.md).
...
- **Backend / API** — In **`windex-api/`** in this directory. Supabase (Postgres, Auth, Edge Functions). Endpoints: ingest event results, get standings, compute money deltas (round-scoped), generate payment requests (round-scoped). Full contract: [windex-api/docs/api.md](./windex-api/docs/api.md).
...
- **UI / front end** — Windex app screens: Dashboard, Rounds, Round Entry, Round Edit, Standings, Groups, Players, Points Analysis, Attribution Review, Player Mapping. Admin UI in **`windex-admin/`** (Vite + React); see that folder’s README. Supports API ingestion, manual round entry, round edit/override, player management, game points analysis (all-vs-all matrix, head-to-head drill-down, Signature Events filtering), and Glide data import. See [Screen Map](./Windex_Screen_Map.md) and [UI Architecture](./Windex_UI_Architecture.md).

---

## Current architecture (what is built)

- **windex-api** (Windex backend; **`windex-api/`** in this directory): Supabase project with migrations, RLS, and Edge Functions.
...
- **Windex docs and admin UI:** This directory (root .md files and **`windex-admin/`**). Source apps that call the API (e.g. Scorekeeper, 18Birdies) are separate apps.
...
Screens and navigation can be designed around these flows; the API contract and error codes are in [windex-api/docs/api.md](./windex-api/docs/api.md).
...
- **windex-api**: Integration tests (`npm run test:integration`) and domain-rule tests (`npm run test:domain`) in **windex-api/**; run against Supabase Cloud (see that folder’s README).
- **windex-admin** and **source apps**: Each has its own test setup.
...
| [Windex_API_Spec.md](./Windex_API_Spec.md) | Endpoint summary; full contract in [windex-api/docs/](./windex-api/docs/). |
| [Windex_Screen_Map.md](./Windex_Screen_Map.md) | Screens and flows. |
| **windex-api/** | Backend/API (Supabase). See [windex-api/README.md](./windex-api/README.md) to run. Points ledger design: [windex-api/docs/POINTS_LEDGER_ARCHITECTURE.md](./windex-api/docs/POINTS_LEDGER_ARCHITECTURE.md). |
...
| [bootstrap-windex-api.md](./bootstrap-windex-api.md) | Bootstrap plan for the API (windex-api/). |
| [windex-api.md](./windex-api.md) | windex-api folder structure (in this directory). |
| [windex-api-CLAUDE.md](./windex-api-CLAUDE.md) | CLAUDE.md template; see [windex-api/CLAUDE.md](./windex-api/CLAUDE.md) for the project file. |
