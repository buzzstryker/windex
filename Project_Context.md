...
- **Cloud-only Supabase.** No local Supabase used. Migrations and functions deploy via `supabase link --project-ref ftmqzxykwcccocogkjhc` then `supabase db push` / `supabase functions deploy --no-verify-jwt`. The `--no-verify-jwt` flag is required because functions handle auth internally.
