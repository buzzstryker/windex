...
- **[2026-05-09] Phone-only group picker on Standings / Rounds / Analysis tab headers (`windex-expo`).** Added `<GroupPicker />` between `<Header />` and `<GroupBanner />` on all three tabs. Renders only when `useWindowDimensions().width < 768` (new convention — no prior breakpoint pattern existed); on tablet/desktop the picker is absent from the tree and group switching uses the existing hamburger drawer. The picker's group list comes from `GroupContext.myGroups` — the user's active `group_members` rows enriched against `groups` and **ordered by `group_members.joined_at` ASC (earliest membership first)**, with name as the tie-break. The drawer's "My Groups" section was switched to the same `myGroups` list (single source of truth); super admins additionally see "Other Groups" in the drawer for cross-org browsing. Single-membership users see static text instead of a dropdown affordance. Manual selection is persisted via the new `lib/userPrefs.ts` (web `localStorage` + native `expo-file-system`, mirroring `authPersistence`) keyed by `selectedGroupId:<user_id>`, and `AuthContext` now exposes `userId` for that purpose. Default-group selection chain: persisted manual choice → earliest-joined of `myGroups` → alphabetical first overall (the third bucket has no `joined_at` to read because the user has no memberships there — super admins / brand-new users). Replaces the previous "most recent active event's group" heuristic. **Schema note:** `group_members.joined_at` already existed (`TIMESTAMPTZ NOT NULL DEFAULT now()`, migration 001); no schema change was needed. The picker's `select=group_id,role,joined_at` query is the only client-side bump. Files: `components/GroupPicker.tsx` *(new)*, `lib/userPrefs.ts` *(new)*, `contexts/AuthContext.tsx`, `contexts/GroupContext.tsx`, `components/Drawer.tsx`, `app/(tabs)/{standings,rounds,analysis}.tsx`.
