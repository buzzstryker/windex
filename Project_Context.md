...
- **[2026-05-09] Phone-only group picker on Standings / Rounds / Analysis tab headers (`windex-expo`).** Added `<GroupPicker />` between `<Header />` and `<GroupBanner />` on all three tabs. Renders only when `useWindowDimensions().width < 768` (new convention — no prior breakpoint pattern existed); on tablet/desktop the picker is absent from the tree and group switching uses the existing hamburger drawer. The picker's group list comes from `GroupContext.myGroups` — the user's active `group_members` rows enriched against `groups` and sorted alphabetically. The drawer's "My Groups" section was switched to the same `myGroups` list (single source of truth); super admins additionally see "Other Groups" in the drawer for cross-org browsing. Single-membership users see static text instead of a dropdown affordance. Manual selection is persisted via the new `lib/userPrefs.ts` (web `localStorage` + native `expo-file-system`, mirroring `authPersistence`) keyed by `selectedGroupId:<user_id>`, and `AuthContext` now exposes `userId` for that purpose. **Schema flag:** the spec asked for ordering by `group_members.created_at`, but that column does not exist — `group_members` has `joined_at` (TIMESTAMPTZ NOT NULL DEFAULT now()) instead. Per spec we fell back to alphabetical ordering and did not add `created_at`; if `joined_at` is the intended source of truth, that's a one-line schema change to revisit. Default group selection logic was also updated: persisted manual choice → alphabetical first of `myGroups` → alphabetical first overall (replaces the previous "most recent active event's group" heuristic). Files: `components/GroupPicker.tsx` *(new)*, `lib/userPrefs.ts` *(new)*, `contexts/AuthContext.tsx`, `contexts/GroupContext.tsx`, `components/Drawer.tsx`, `app/(tabs)/{standings,rounds,analysis}.tsx`.
