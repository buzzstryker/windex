...
- **Login activity viewer in windex-admin.** Add a "Login Activity" tab or section that surfaces `public.login_events` for super admins. Likely shape: a table joined to `players`, filterable by player, event_type, and date range, ordered by `occurred_at desc`. Pagination if the table grows. No write actions — read-only. Backend (table, triggers, RLS) already exists from migration 022.
