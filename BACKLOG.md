...
- **Activity viewer in windex-admin — covers both login_events and user_events.** Add an "Activity" admin screen for super admins that surfaces the two audit tables side-by-side (likely two tabs or two filtered views in one screen): `public.login_events` from migration 022 (auth-layer: `login_success` / `token_refresh` / `logout` with IP and user_agent) and `public.user_events` from migration 023 (app-layer: `group_switch` / `view_leaderboard` / `view_rounds_list` with group_id and season_id). Both tables joined to `players` for display names, filterable by player + event_type + date range, ordered by `occurred_at desc`. Pagination if either table grows. No write actions — read-only. Backend (tables, RLS gating SELECT to super admin) already exists for both. **Naive "views per group" queries on user_events should filter `event_type = 'view_leaderboard'` explicitly** or de-dup within a short window, because a group switch fires `group_switch` + a view_* event per currently-mounted tab (typically 3 rows within ~2ms) — see `windex-expo/lib/userEvents.ts` header for the pairing details.
