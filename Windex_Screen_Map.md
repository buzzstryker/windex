...
- **Expo app (windex-expo) tab bar** — 3 tabs: Standings, Rounds, Analysis. Olive green header. Hamburger drawer for group selection (My Groups / Other Groups with logo thumbnails + checkmark), Group Details, Sign out. **The centered title in the header doubles as the phone group picker on all three tabs:** at `width < 768px` with multiple memberships it reads `Standings ▾ Windex Cup` and the whole unit is tappable (opens a modal list of `myGroups`). Single-membership phone users see `Standings · Windex Cup` as static text — no chevron, no tap. Tablet/desktop (`>= 768px`) shows just the tab name centered and group switching falls back to the drawer. The picker is composed inside `Header`'s title slot via `<Header title={<GroupPicker tabName="Standings" />} />` — `Header` accepts `string | ReactNode` and `GroupPicker` always renders the tab name (tab-only on desktop, "tab · group" static on phone-single, pressable "tab ▾ group" on phone-multi). Picker scopes to active `group_members` rows ordered by `group_members.joined_at` DESC (most-recently-joined first; default-selected group on app load follows the same direction so a player invited to a new group lands on that group on next sign-in). Default-group chain: persisted manual choice → most-recently-joined of `myGroups` → alphabetical-first overall (no-membership fallback only — super admin / brand-new user with no `group_members` rows). Resolution is gated on memberships and persisted choice both being loaded; consumers show a spinner instead of flashing the wrong group then correcting. Manual selection persists per `user_id`. Both surfaces share `GroupContext.myGroups` and stay in sync.
...
| **Group detail** | Per-group view: list of seasons, "+ Create Season" button (super admin only — bootstraps the seed season for a brand-new group, since the daily auto-rollover has nothing to extrapolate from until one season exists), and a Danger Zone delete-group control (super admin only). Auto-rollover (migration 021 + pg_cron daily at 06:00 UTC) creates the next season ahead of time; the user-facing picker hides future-dated seasons until their `start_date` arrives. Admin's seasons list intentionally shows them so super admins can verify rollover output. Route: `/groups/:groupId`. |
